...
| Profile | Arguments |
|---|---|
| `web` | `--auth`, `--host`, `--port`, `--tls-cert`, `--tls-key`, `--public-url`, repeatable `--trusted-host` |
...
`dsh web` is a hardcoded alias for `--profile web`; the flags after it belong to the web app, whose ordinary bundle provider parses them. `--host` and `--port` override the composed values of the rows that carry them. `--auth required` resolves `DSH_WEB_TOKEN`, accepts it only on `/auth/login`, and grants an HttpOnly same-origin session cookie; the token is not written to browser storage. `--tls-cert` and `--tls-key` enable built-in HTTPS and must be supplied as a pair. `--public-url https://…` names the external HTTPS URL when a reverse proxy terminates TLS; its authority joins the `/api` browser-trust fence. Repeatable `--trusted-host` adds further named authorities. `--host 0.0.0.0` requires `--auth required` and either paired built-in TLS files or an HTTPS public URL. The client-plugin HMR receiver is always mounted and stays idle until a separate `pnpm run dev:web` watcher rebuilds client bundles.
...
The production Web runner needs built package and frontend artifacts (`pnpm run build`). It serves `http://127.0.0.1:3080` without authentication by default. A remote deployment sets `DSH_WEB_TOKEN` in the launch environment or managed credential store, then uses either built-in TLS or a reverse proxy with an explicit public HTTPS URL. `--trusted-host` adds named authorities accepted by the `/api` browser-trust fence; it is a DNS-rebinding defense, not authentication.
