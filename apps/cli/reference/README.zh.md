...
| Profile | 参数 |
|---|---|
| `web` | `--auth`、`--host`、`--port`、`--tls-cert`、`--tls-key`、`--public-url`、可重复的 `--trusted-host` |
...
`dsh web` 是 `--profile web` 的硬编码别名；写在它之后的 flag 属于 web 应用，由组合包中的普通提供方解析。`--host` 和 `--port` 覆盖承载它们的那些行的组合取值。`--auth required` 解析 `DSH_WEB_TOKEN`，只在 `/auth/login` 接受它，并授予 HttpOnly 同源会话 Cookie；令牌不会写入浏览器存储。`--tls-cert` 和 `--tls-key` 启用内建 HTTPS，必须成对提供。`--public-url https://…` 指定反向代理终止 TLS 时的外部 HTTPS URL，其 authority 会加入 `/api` 浏览器信任围栏。可重复的 `--trusted-host` 添加更多具名 authority。`--host 0.0.0.0` 要求 `--auth required`，且必须提供成对的内建 TLS 文件或 HTTPS public URL。客户端插件 HMR（热模块替换）接收器始终挂载，在单独运行的 `pnpm run dev:web` watcher 重建客户端 bundle 之前保持空闲。
...
生产 Web 运行器需要已构建的包和前端产物（`pnpm run build`）。默认以无认证方式服务 `http://127.0.0.1:3080`。远程部署在启动环境或受管凭据存储中设置 `DSH_WEB_TOKEN`，再使用内建 TLS，或使用带明确 HTTPS public URL 的反向代理。`--trusted-host` 可添加 `/api` 浏览器信任围栏接受的具名 authority；它是 DNS 重绑定防护，不是认证。
