...
// credential is absent, the inline key form is the only first-run dialog, and
// the write lands in an isolated harness home without a reload or model call.
...
describe.skipIf(MODE === 'record')('web e2e: first-run DeepSeek credential setup', () => {
  ...
  it('never paints the takeover chrome on a configured reload, even with the settings join held open', async () => {
    ...
    // but must load private facts before deciding not to show. Dialog chrome
    // lives inside its visible branch, so the deciding window paints and blocks
  ...
  it('configures arbitrary DeepSeek models and prompts after the selected model is removed', async () => {
    ...
    // after configuring the key, so no settings dialog remains open.
    await page.getByRole('button', { name: '设置', exact: true }).click()
    const settings = page.getByRole('dialog', { name: '设置' })
    await settings.waitFor({ timeout: 10_000 })
    await settings.getByRole('button', { name: '模型' }).click()
