...
`host` accepts only `127.0.0.1` (default posture) and `0.0.0.0` (deliberate network exposure). Pairing `tlsCert` and `tlsKey` creates an HTTPS listener; a lone field fails activation. Authentication and browser-origin policy belong to composing plugins. The dist location is an assembly fact of the frontend plugin that claims the seat.
