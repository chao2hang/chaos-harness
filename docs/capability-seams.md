...
| `ctx.webAuth` | `core` | `web-auth` | - | `connection` | - | Verifies the session Cookie for routes that authorize authenticated remote operations after transport admission. |
| `ctx.webAuthToken` | `core` | `web-auth` | - | `webserver`, `web-auth` | - | Resolves the deployment token before the listener activates; the auth plugin consumes the process-local result to protect HTTP and WebSocket traffic. |
