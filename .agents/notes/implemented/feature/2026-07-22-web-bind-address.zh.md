...
`dsh web` 默认绑定 `127.0.0.1`。CLI（命令行界面）接受 `--host 0.0.0.0` 作为显式启用的全接口模式，并拒绝其他取值，使网络模式保持为一份规模小、经过审慎限定的约定。全接口模式要求认证 HTTPS，详见[远程 Web 部署](2026-08-16-web-remote-deployment-authentication.md)，在未配置反向代理 public URL 时仍然输出本机环回 URL 和第一个外部 IPv4 URL。
