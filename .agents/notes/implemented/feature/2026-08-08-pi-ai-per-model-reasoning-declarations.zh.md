...
- 模型设置卡片公开按模型的标准档位与 DeepSeek 档位预设。新添加的自定义模型从标准声明起步，运维人员可在保存前将其标记为不可调节，或收窄为 DeepSeek 的 `off`／`high`／`max` 集合。`resolveModelInfo` 经输入框档位面板所用的同一 catalog 元数据报告最终档位。
- composer 把从 `off` 到 `max` 的固定升级顺序渲染为滑块，将不支持的位置标为不可用，并把落在不可用位置的操作吸附到最近较低的受支持档位。切换模型时同样会在目标支持当前显式档位时保留，否则选择双方共有的最近较低档位并提示这次调整。目标完全没有推理元数据时会清除显式档位，交给提供方默认行为。Host 仍会在提供方 I/O 前拒绝不受支持的显式 id；它绝不会从被压平的提供方错误文本中猜测，也不会换档重试已经失败的请求。
- #1860 暂缓的缺口——模型接不住的路由级档位会让发往它的请求失败——有了运维侧补救：对齐该模型的 `reasoningEfforts`，或去掉路由默认值。
