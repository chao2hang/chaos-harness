...
When a confining executor is mounted, `bash` advertises paired `sandbox_permissions` and `justification` fields. The schema exposes the full closed escalation vocabulary because effective mode is per-session; a strictly wider target still needs approval before execution, and a non-widening target is ignored so the call keeps the standing mode ([redundant non-widening asks](../bug-fix/2026-08-17-ignore-non-widening-sandbox-permissions.md)). `allowed-once` stamps the granted mode onto only that request, while `rejected`, `cancelled`, `unavailable`, a missing approval service, or a missing agent all fail closed with distinct results. No grant is persisted.
