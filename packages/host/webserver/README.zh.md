...
Web HTTP/HTTPS 与 upgrade route 注册插件（默认导出 `WebServer`，配置为 `{host, port, tlsCert?, tlsKey?}`），在激活时监听并提供 `ctx.webServer`。成对配置 `tlsCert` 和 `tlsKey` 时读取 PEM 文件并创建 HTTPS 监听器；任一字段单独存在都会使激活失败。`protocol` 报告活动传输为 `http` 或 `https`。`register(route)` 添加具名的 `exact`／`prefix` HTTP route；`registerUpgrade(route)` 添加精确 pathname 的 upgrade route。`registerGuard(guard)` 和 `registerUpgradeGuard(guard)` 会在各自的 route 分发前按注册顺序运行；返回 false 时，guard 自己负责响应或 socket。同一张路由表内的重复路径会抛错，因为 route 模式是组合层约定，冲突即配置错误；注册方法都会返回 disposer。`registerFallback(handler)` 注册一个 handler，处理所有未被具名 route 命中的请求。第二次注册会抛错；随附的 SPA dist 服务器 [`dsh-host-frontend-static`](../frontend-static/README.md) 是该 handler 的所有者，没有注册 handler 时服务器返回 404。`tapIndex(transform)` 添加一个 index.html 转换，`applyIndexTaps(html)` 按注册顺序对一段响应体运行已注册的转换；fallback handler 在每次 index 响应时调用它。`port` 读取正在监听的端口（当 `port` 为 0 时读取 OS 分配的值），`host` 读取配置的绑定宿主（这些是其他插件据以自适应的组合期事实，例如 directory-picker 选择器）。HTTP 匹配顺序固定不变：先在整张表中匹配精确 route，再匹配最长前缀，最后交给 fallback handler。upgrade 只做精确匹配，未命中连接直接关闭；注册顺序不影响请求处理。
...
- **认证由独立插件负责**：`dsh-host-web-auth` 持有令牌验证与浏览器会话；此载体只执行注册。
