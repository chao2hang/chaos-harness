...
`host.restart` 用相同命令行启动的后继进程替换当前服务进程，这是让声明了 `applies: 'restart'` 的设置、已组装插件集合与重新构建的模块代码生效的途径。网关自身不执行重启：它通过可选的 `ctx.appRestart` 值（[`dsh-cmdline`](../../boot/cmdline/README.md)）请求启动器，而 `host.describe.canRestart` 宣告该值是否存在，因此启动方式无法拉起后继进程的部署会回答 `restart-unavailable`，而不是退出后无人接手。该响应是一次应答，而非完成报告——在调用内部拆掉自己的服务器根本无法作答——因此网关先回复，`restartGraceMs`（默认 250 毫秒；跨越比环回更慢的链路时应调高）之后才请求启动器。正确性并不依赖这段宽限：无论如何调用方都会把这次交接观察为一次普通断连，而所有会话、下行连接与进行中的轮次都随前一个进程结束。因任何其他原因发生的拆卸都会取消待执行的交接，因此宽限期内的信号不会留下无人请求的后继进程。该方法在 [`dsh-client-connection`](../../client/connection/README.md) 中受认证约束而非固定在环回地址：全网卡部署恰恰是没人能够触及终端把服务重新拉起来的场景，而已认证的调用方本就能通过任意会话的工具以该进程的身份执行命令。
