...
- `ctx.appRestart`：一个有边界的进程替换请求：先拆卸配置树，再用同一条命令行引导后继进程。

没有命令行的嵌入宿主提供空列表；这是诚实的答案，而不是缺失的值。

`restart` 是唯一可选的成员，因为无法启动后继进程的启动器根本给不出诚实的实现：嵌入宿主，或生命周期由监管进程持有的界面。它于是省略 `restart`，`ctx.appRestart` 保持缺失，消费方据此报告该能力不可用。在那里改用普通退出，只会让服务停下而没有任何东西接手。
