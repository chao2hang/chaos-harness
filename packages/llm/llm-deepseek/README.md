...
- **Reasoning passback rule**: every historical assistant turn with non-empty reasoning serializes it back as `reasoning_content`, including plain-text, tool-call, and reasoning-only turns. Thinking-mode gateways may reject the next request if any prior reasoning value is missing.
...
The selected DeepSeek model receives the harness system prompt, message history, tool schemas, stop sequences, and call config without adapter-authored prompt prose. Every prior assistant turn with non-empty reasoning passes that value back as `reasoning_content`, whether the turn contains text, tool calls, or reasoning alone.

#### Token effect

Provider tokenization governs exact input. Reasoning passback increases later thinking-mode requests by the retained reasoning text; cache-read usage is reported when available.

#### KV Cache effect

An unchanged assembled prefix is eligible for DeepSeek cache reuse, which this adapter reports in usage. A model-route change or any upstream prompt, schema, prefix, or history change may prevent reuse from the first changed token; each retained assistant reasoning value remains part of that history prefix.
