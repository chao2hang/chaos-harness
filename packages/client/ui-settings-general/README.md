...
The Host half keeps `ui-onboarding.welcomeNoticeVersion` registered in the user-settings seam so existing `settings.yaml` documents remain valid. API Proxy does not expose the namespace, and no browser plugin reads or writes that historical acknowledgement field.
