...
宿主端继续在用户设置 seam 中注册 `ui-onboarding.welcomeNoticeVersion`，使既有 `settings.yaml` 文档保持有效。API Proxy 不暴露该 namespace，浏览器插件也不再读写这项历史确认字段。
