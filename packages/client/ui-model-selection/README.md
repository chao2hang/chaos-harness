...
Model selection plugin, browser half: TWO entries over ONE per-session directory owned by `ModelDirectoryResolver` (`ctx.modelDirectories`). For ordinary sessions, the `/model` popupSelect contribution (registered through `ctx.commandUi`) and the composer's named `conversation.input.model` seat both load the session's advisory directory through `session.models` and submit through `session.selectModel` via the same `ModelDirectory` instance. The compact composer trigger opens a parameter menu for the provider-grouped model list, a fixed off-to-MAX reasoning slider, context capacity, and maximum output tokens. The selected exact model supplies its adapter-owned effort names, defaults, and capacities. `/model` applies the selected model's default effort; the composer can choose an advertised effort, and an explicitly configured pi-ai model permits its capacities to be changed and persisted globally from the same picker.
