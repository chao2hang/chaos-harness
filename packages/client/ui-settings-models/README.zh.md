...
模型设置与凭据引导插件。同一个 client Cordis 插件会注册 Models 页面，并通过 `settings.onboarding` 注册一个按条件显示的 DeepSeek 官方凭据弹窗；它不会显示产品声明或内测声明。Models 平面把三个协议领域汇聚为一个共享快照：`llm.providers`（可配置提供方目录，含每条路由的存活／休眠状态）、`settings.describe`（序列化 schema、分层脱敏值、secret slot）与 `credentials.describe`（不含值的 configured/source/writable 徽标）；页面据此渲染提供方行，一次只展开一张编辑卡片，且不把路由存活状态呈现为提供方状态。
...
pi-ai profile 的 `models` 列表就在卡片上编辑：一行一个模型，行上显示 id 与显示名称，能力声明与精确容量输入收在该行的展开区内，右侧是两个无文字的操作——展开与删除。新添加或从端点发现的自定义模型默认声明文本与图片输入，以及标准 `off`／`minimal`／`low`／`medium`／`high`／`xhigh`／`max` 推理档位；保存前可在展开区把它标为仅文本、不可调节，或收窄成 DeepSeek 的 `off`／`high`／`max` 声明。这些是由适配器强制执行的端点能力声明，不是从失败请求中猜出的能力。空列表意味着「使用该路由的内置 catalog」，因此每一行都只会被刻意添加；清空容量会丢弃它，而不是存入一个 schema 会拒绝的值，配置留空的部分由适配器的路由级回退值定尺寸——留空的容量以这些回退值的量级作为占位符，那只是提示而非镜像：该字段按 1000 计 `K`，且部署可以覆盖这些回退值。不是正整数的容量根本不会被存下。
